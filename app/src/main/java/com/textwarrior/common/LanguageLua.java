 ...
 */
package com.textwarrior.common;
...
public class LanguageLua extends Language {
	...
	private final static String keywordTarget ="and|break|case|continue|default|defer|do|else|elseif|end|false|for|function|goto|if|in|lambda|local|nil|not|or|repeat|return|switch|then|true|until|when|while|EXIT";
	...
	public final static Map<String,String[]> classfunc = new HashMap<>();
	...
	private LanguageLua(){
	    ...
	    if(classfunc!=null){
		    for(String name: classfunc.keySet()){
				super.addBasePackage(name,classfunc.get(name));
			}
	    }
	}
